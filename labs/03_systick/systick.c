...
int main( void)
{
    ...
    // Эксперимент:
    //
    //   Период биений: T = (280 +- 10) мин
    //
    //   (1с + dt) * N = 1 сек * (N + 1), где N = T/1 сек
    //   => dt = 1 сек / T = 60 мкс
    //
    //   Кол-во тактов разницы = CPU_FREQENCY * dt = 2880 +- 200
    //   Такты на stacking/unstacking = 32 * 100 = 3200 > 2880
    //
    // Выводы:
    //
    //   Эксперимент показывает, что биения обусловлены stacking-ом/unstacking-ом.
    //   И не позволяет точно определить погрешность delay_ms.
    //
    // Обсуждения:
    //
    //   Зачем-то экспериментатор сделал stacking 100 раз в секунду.
    //   Возможно, следует убрать его полностью, или хотя бы уменьшить до 1 раза в секунду.
    //   Время наблюдения биений тогда предположительно составит порядка 500 часов.
    //
    for ( ;; )
    {
